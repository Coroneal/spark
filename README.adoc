# Spring boot and app hot starter

image:https://travis-ci.org/geowarin/boot-app.svg?branch=master["Build Status", link="https://travis-ci.org/geowarin/boot-app"]
image:https://ci.appveyor.com/api/projects/status/y3uw0gpo9dtec349?svg=true["Windows build Status", link="https://ci.appveyor.com/project/geowarin/boot-app"]
...
and https://github.com/gaearon/babel-plugin-app-transform[app transform] for hot reloading.

Everything: backend, frontend and styles will be hot reloaded automatically.

See http://geowarin.github.io/spring-boot-and-app-hot.html[my article] for an in-depth explanation.
...
There is a https://github.com/geowarin/boot-app/tree/groovy[groovy branch] that should be up to date.
...
The node version used by the gradle build is specified https://github.com/geowarin/boot-app/blob/master/frontend/build.gradle#L11-L12[here].
...
See: https://github.com/geowarin/boot-app/blob/master/backend/src/main/java/app/config/redis/EmbeddedSessionConfig.java[EmbeddedSessionConfig].
...
```
java -jar boot-app-0.0.1-SNAPSHOT.jar --spring.profiles.active=production                                                       16:57:01
```
...
This project uses https://github.com/rackt/app-redux[Redux] to handle state and actions.
...
I also suggest reading the https://github.com/rackt/app-redux/blob/master/docs/quick-start.md[redux quick start] to understand
...
Small components are written using the https://facebook.github.io/app/blog/2015/10/07/app-v0.14.html#stateless-functional-components[stateless functional components syntax], i.e,
those component are pure render components and only their props will have an impact on the DOM.

Beware, hot reloading is not supported on functional components yet (see https://github.com/geowarin/boot-app/issues/13[this issue]) unless they are wrapped in a real Component.
...
The project uses https://github.com/rackt/app-router[app-router] to handle routes.
...
https://github.com/geowarin/boot-app/blob/master/frontend/server.js#L21-L24[proxies] requests to the index.

In production, we have to use a special https://github.com/geowarin/boot-app/blob/master/backend/src/main/java/app/config/SinglePageAppConfig.java[resource handler]
...
If you want to include static assets like images in the project, please see https://github.com/geowarin/boot-app/issues/16[this issue],
...
See the https://github.com/geowarin/boot-app/blob/master/backend/src/test/groovy/app/auth/AuthenticationSpec.groovy[auth-spec]
for an example.
...
```
java -jar build/libs/boot-app-0.0.1-SNAPSHOT.jar
```
...
And it will create a docker image named `boot-app/boot-app`.

```
> docker images
REPOSITORY                               TAG                 IMAGE ID            CREATED              VIRTUAL SIZE
boot-app/boot-app                    latest              5280d39f660f        About a minute ago   138.9 MB
```

You can then run it with:

```
docker run -p 8080:8080 boot-app/boot-app
```

You can also pass arguments to the application like this:

```
docker run -p 8080:8080 boot-app/boot-app --spring.profiles.active=redis --spring.redis.host=redis
```
